...
pub fn connect_to(url: &str, token: &str, channelid: u64, pings: &Vec<u64>) {
    let resc = connect(Url::parse(url).unwrap());
    ...
    let hbintervalasu128: u128 =
        u128::try_from(re.d.get("heartbeat_interval").unwrap().as_u64().unwrap())
            .expect("Heartbeat could not be converted to u128");
    ...
    println!(
        "[{:02}.{:02}.{:02}] - First Heartbeat sent at {:02}:{:02}:{:02}!",
        Local::now().day(),
        Local::now().month(),
        Local::now().year(),
        Local::now().hour(),
        ...
        MaybeTlsStream::NativeTls(t) => {
            t.get_mut()
                .set_read_timeout(Some(std::time::Duration::from_millis(
                    (hbdata.heartbeat_interval as f64 / 1.2) as u64,
                )))
    ...
    let authdata: String = format!(
        r#"{{"op": 2,"d": {{"token": "{}","properties": {{"os": "linux","browser": "firefox","device": "pc"}}}}}}"#,
        token
    );
    let authpayload = Message::Text(authdata);
    socket
        .write_message(authpayload)
        .expect("Failed to send auth payload");
    let auth_msg = match socket.read_message() {
        Ok(msg) => msg,
        Err(_err) => Message::Text(format!("")),
    };

    if auth_msg.is_empty() {
        panic!("No Authentication response received.");
    }

    let auth_response: Result<UsualRoot, Error> = serde_json::from_str(auth_msg.to_text().unwrap());

    match auth_response {
        Ok(authdata) => {
            println!(
                "[{:02}.{:02}.{:02}] - Authentication type received: {}!",
                Local::now().day(),
                Local::now().month(),
                Local::now().year(),
                authdata.t
            ...
            if authdata.t == "READY" {
                println!(
                    "[{:02}.{:02}.{:02}] - Authentication successful.",
                    Local::now().day(),
                    Local::now().month(),
                    Local::now().year()
                );
                ...
                println!(
                    "[{:02}.{:02}.{:02}] - Has the authentication failed?",
                    Local::now().day(),
                    Local::now().month(),
                    Local::now().year()
                );
                let ignore = read_input(format!(
                    "[{:02}.{:02}.{:02}] - Ignore failed parse? Probably means authentication failed! [Y/N]",
                    Local::now().day(),
                    Local::now().month(),
                    Local::now().year()
                ).as_str());
                if ignore == "Y" || ignore == "y" {
                    println!(
                        "[{:02}.{:02}.{:02}] - Parsing error ignored.",
                        Local::now().day(),
                        Local::now().month(),
                        Local::now().year()
                    );
                    ...
                    println!(
                        "[{:02}.{:02}.{:02}] - Authentication failed!",
                        Local::now().day(),
                        Local::now().month(),
                        Local::now().year()
                    );
        ...
        Err(_err) => {
            println!(
                "[{:02}.{:02}.{:02}] - Unparsable authentication message received.",
                Local::now().day(),
                Local::now().month(),
                Local::now().year()
            );
    ...
    loop {
        ...
        if !msg2.is_empty() {
            if msg2.to_string().contains("\"t\":\"MESSAGE_CREATE\",") {
                let msgrecvobj: Result<GuildMessageRoot, Error> =
                    serde_json::from_str(msg2.to_text().unwrap());
                match msgrecvobj {
                    Ok(_msgobj) => {
                        println!(
                            "[{:02}.{:02}.{:02}] - Was a guild message - ignored",
                            Local::now().day(),
                            Local::now().month(),
                            Local::now().year()
                        );
                    ...
                    Err(_err) => {
                        let personalmessageobj: Result<MessageRoot, Error> =
                            serde_json::from_str(msg2.to_text().unwrap());
                        match personalmessageobj {
                            Ok(msgobj) => {
                                ...
                                let msg1: MessageObj = message::MessageObj::new(format!(
                                    ...
                                    pingstr,
                                    msgobj.d.author.id,
                                    msgobj.d.author.username,
                                    msgobj.d.author.discriminator,
                                    msgobj.d.content
                                ));
                                sendmessage(msg1, token, channelid);
                                sendmessage(
                                    message::MessageObj::new(format!(
                                        "?ban <@{}> 0 \"Sent a dm! Likely spam\"",
                                        msgobj.d.author.id
                                    )),
                                    token,
                                    channelid,
                            ...
                            Err(_err) => {
                                let usualobj: Result<UsualRoot, Error> =
                                    serde_json::from_str(msg2.to_text().unwrap());
                                match usualobj {
                                    Ok(obj) => {
                ...
                match usualobj {
                    ...
                    Err(_err) => {
                        let ignoretypeobject: Result<FailsafeRoot, serde_json::Error> =
                            serde_json::from_str(msg2.to_text().unwrap());
                        match ignoretypeobject {
                            Ok(msg) => println!(
                                "[{:02}.{:02}.{:02}] - Failsafe Message received: OP Code = {}",
                                Local::now().day(),
                                Local::now().month(),
                                Local::now().year(),
                                msg.op
                            ...
                            Err(_err) => {
                                println!(
                                    "[{:02}.{:02}.{:02}] - Error parsing response... {}",
                                    Local::now().day(),
                                    Local::now().month(),
                                    Local::now().year(),
                                    _err
        ...
        println!(
            "[{:02}.{:02}.{:02}] - Heartbeat sent at {:02}:{:02}:{:02}!",
            Local::now().day(),
            Local::now().month(),
            Local::now().year(),
            Local::now().hour(),
